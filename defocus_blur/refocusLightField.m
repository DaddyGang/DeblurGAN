...
function [Iout lightFieldOut] = refocusLightField(lightField, alpha)
    ...
    for ky=1:size(lightField,1)
        for kx=1:size(lightField,2)
            ...
            for k=1:3
                I(:,:,k) = interp2(XX,YY,II(:,:,k), XX + (alpha-1)*(kx-(floor(size(lightField,2)/2)+1)), YY - (alpha-1)*(ky-(floor(size(lightField,1)/2)+1)), 'linear', 1);
            end
